#Project - Reinforcement Learning applied to Black Jack using Monte Carlo Methods
...
#However, compared to assignment we will take into consideration the natural 21 and
#we will also take into consideration counting cards. The method used to count cards 
...
for(i in 1:n)
{
  ...
  for(j in 1:(length(rewards)-1))
  {
    ...
    #Get state action pairs for time t+1
    x1<-totalsplayer[j+1]-11
    y1<-visibledealer[j+1]-1
    z1<-usableacesplayer[j+1]+1
    k1<-2*cardcounts[j+1]+7   
    l1<-actions[j+1]
    
    #Update Q and N matrix for state picked
    #ActionFunctionEstim[x,y,z,k,l]<-PreviousQ[x,y,z,k,l]+alpha*(rewards[j]+discount*PreviousQ[x1,y1,z1,k1,l1]-PreviousQ[x,y,z,k,l])
    #ActionFunctionEstim[x,y,z,k,l]<-PreviousQ[x,y,z,k,l]+alpha*(rewards[j]-PreviousQ[x,y,z,k,l])
...
for(i in 1:10)
{
  ...
  policy5nousable[1,i]<-sum(optimalpolicy[i,4,1,]==1)
  policy5nousable[2,i]<-sum(optimalpolicy[i,4,1,]==2)
  
  #Get the number of 1 and the number of 2 in optimal policy
  policy5usable[1,i]<-sum(optimalpolicy[i,4,2,]==1)
  policy5usable[2,i]<-sum(optimalpolicy[i,4,2,]==2)
  ...
  #Get the number of 1 and the number of 2 in optimal policy
  policyallnousable[1,i]<-sum(optimalpolicy[i,,1,]==1)
  policyallnousable[2,i]<-sum(optimalpolicy[i,,1,]==2)
...
legend((n-3*n/10), 0.2, legend=c('Count = 0', 'count = -1', 'count = 1', 'All counts'),col=c("green","blue", "red", 'purple'), lty=1:2, cex=0.8)
